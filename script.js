...
(function() {
  ...
  artist: "Poseidon",
  album: "Album 2",
  albumArtUrl: "image/Jadilah Juara.png",
...
// ===== ALBUMS ORDER =====
  const albums = ["Album 1", "Album 2", "Album 3", "Album Tentatif"];
  ...
  function getNextSongIndex(currentIndex) {
    ...
    // Cari lagu berikutnya dalam album yang sama
    ...
    // Jika tidak ada, cari album berikutnya
    ...
    if (currentAlbumIndex < albums.length - 1) {
      ...
      for (let i = 0; i < songs.length; i++) {
        if (songs[i].album === nextAlbum) {
          return i;
        }
      }
    }

    // Jika tidak ada album berikutnya, kembali ke awal album pertama
    return 0;
  }
  ...
  function getPrevSongIndex(currentIndex) {
    ...
    // Cari lagu sebelumnya dalam album yang sama
    ...
    // Jika tidak ada, cari album sebelumnya
    const currentAlbumIndex = albums.indexOf(currentAlbum);
    if (currentAlbumIndex > 0) {
      const prevAlbum = albums[currentAlbumIndex - 1];
      for (let i = songs.length - 1; i >= 0; i--) {
        if (songs[i].album === prevAlbum) {
          return i;
        }
      }
    }

    // Jika tidak ada album sebelumnya, kembali ke akhir album terakhir
